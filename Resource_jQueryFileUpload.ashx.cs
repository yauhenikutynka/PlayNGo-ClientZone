...
namespace Playngo.Modules.ClientZone
{
    public class Resource_jQueryFileUploadHandler : IHttpHandler
    {
		...
		private static void ReturnOptions(HttpContext context) {
			context.Response.AddHeader("Allow", "DELETE,GET,HEAD,Event,PUT,OPTIONS");
		...
		private void ListCurrentFiles (HttpContext context) {
            ...
            //int RecordCount = 0;
            //qp.Where.Add(new SearchParam(Playngo_ClientZone_Files._.AlbumID, WebHelper.GetStringParam(context.Request, "AlbumID", "0"), SearchType.Equal));

            //var files = Playngo_ClientZone_Files.FindAll(qp, out RecordCount).Select(f => new Resource_FilesStatus(f, PortalSettings)).ToArray();
