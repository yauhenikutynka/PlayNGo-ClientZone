...
namespace Playngo.Modules.ClientZone
{
    ...
    public class MessageTips
    {
        ...
        /// 向页面注册提示信息
        /// </summary>
        public String Event(DotNetNuke.Entities.Modules.PortalModuleBase Pmb)
        {
            return Event(Pmb.Page);
        }
        ...
        /// <returns></returns>
        public String ViewContentToHtml(String sContent)
        {
            if (!String.IsNullOrEmpty(sContent))
            {
                ...
                if (MsgType == EnumTips.Alert)
                {
                    ...
                    sb.AppendFormat("<i class=\"fa fa-info-circle\"></i> {0} ", sContent).AppendLine();
                ...
                else if (MsgType == EnumTips.Error)
                {
                    ...
                    sb.Append("<button data-dismiss=\"alert\" class=\"close\">×</button>").AppendLine();
                    sb.AppendFormat("<i class=\"fa fa-times-circle\"></i> {0} ", sContent).AppendLine();
                    sb.Append("</div>").AppendLine();
                }
                else if (MsgType == EnumTips.Warning)
                {
                    ...
                    sb.Append("<button data-dismiss=\"alert\" class=\"close\">×</button>").AppendLine();
                    sb.AppendFormat("<i class=\"fa fa-exclamation-triangle\"></i> {0} ", sContent).AppendLine();
                    sb.Append("</div>").AppendLine();
                }
                else if (MsgType == EnumTips.Success)
                {
                    ...
                    sb.Append("<button data-dismiss=\"alert\" class=\"close\">×</button>").AppendLine();
                    sb.AppendFormat("<i class=\"fa fa-check-circle\"></i> {0} ", sContent).AppendLine();
                    sb.Append("</div>").AppendLine();
                }
                return sb.ToString();
            }
