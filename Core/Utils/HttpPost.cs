...
namespace Playngo.Modules.ClientZone
{
    ...
    public class HttpEvent
    {
        ...
        public String Event()
        {
            ...
            if (!String.IsNullOrEmpty(FormatFormString))
            {
                byte[] EventData = encoding.GetBytes(FormatFormString);//FormString即为发送的数据
                request.ContentLength = EventData.Length;
                Stream requestStream = request.GetRequestStream();
                requestStream.Write(EventData, 0, EventData.Length);
