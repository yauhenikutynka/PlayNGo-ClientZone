...
namespace Playngo.Modules.ClientZone
{
    ...
    public class Pager 
    {
        ...
        public  string CreateHtml()
        {
            ...
            if (PageIndex != 1)
            {
                left = String.Format("{0}<a class=\"first button\" href=\"{1}\">{2}</a>", left, CreateUrl(1), Localization.GetString("First", DotNetNuke.Services.Localization.Localization.SharedResourceFile));
                left = String.Format("{0}<a class=\"previous button\" href=\"{1}\">{2}</a>", left, CreateUrl(PageIndex - 1), Localization.GetString("Previous", Localization.SharedResourceFile));


            }
            else
            {
                left = String.Format("{0}<span class=\"first disabled\">{1}</span>", left, Localization.GetString("First", Localization.SharedResourceFile));
                left = String.Format("{0}<span class=\"previous disabled\">{1}</span>", left, Localization.GetString("Previous", Localization.SharedResourceFile));
            }

            //显示尾页 下一页
            if (PageIndex < pageCount && pageCount > 1)
            {
                right = String.Format("{0}<a class=\"next button\" href=\"{1}\">{2}</a>", right, CreateUrl(PageIndex + 1), Localization.GetString("Next", Localization.SharedResourceFile));
                right = String.Format("{0}<a class=\"last button\" href=\"{1}\">{2}</a>", right, CreateUrl(pageCount), Localization.GetString("Last", Localization.SharedResourceFile));
            }
            else
            {
                right = String.Format("{0}<span class=\"next disabled\">{1}</span>", right, Localization.GetString("Next", Localization.SharedResourceFile));
                right = String.Format("{0}<span class=\"last disabled\">{1}</span>", right, Localization.GetString("Last", Localization.SharedResourceFile));
            }
            ...
            for (int i = min; i <= max; i++)
            {
                ...
                {
                    center += "<span class=\"index disabled\">[" + i + "]</span>";
                }
                else
                {
                    center += "<a class=\"index button\" href=\"" + CreateUrl(i) +"\">" + i + "</a>";
                }
            }
            ...
            html.AppendFormat("<span class=\"total\">{0}</span> {1} {2} {3}", total, left, center, right);
            html.Append( "</div>");
